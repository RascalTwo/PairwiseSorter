...
const { SESSION_SECRET, NODE_ENV } = require('./config/constants');
...
module.exports = function createServer(client) {
	...
	app.use(expressSession({
		...
		store: NODE_ENV === 'production' ? MongoStore.create({ client }) : new (require('session-file-store')(expressSession))({ path: './dev-sessions'})
	}));
	setupPassport(app);
